...
module.exports = () => {
    ...
    api.get('/todo', async (req, res) => {
        try {
            ...
            res.status(200).json({ status: 'success', payload: allTod, message: 'All todos fetched successfully' });

        } catch (err) {
            res.status(200).json({ status: 'success', payload: , message: 'All courses fetched successfully' });
        }
    });
