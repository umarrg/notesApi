...
module.exports = () => {
    ...
    api.get('/note', async (req, res) => {
        try {
            ...
            res.status(200).json({ status: 'success', payload: allNotes, message: 'All notes fetched successfully' });

        } catch (err) {
            res.status(200).json({ status: 'success', payload: allNotes, message: 'error' });
        }
    });
    ...
    api.get('/note/:id', async (req, res) => {
        ...
        if (id) {
            try {
                ...
                res.status(200).json({ status: 'success', payload: singlenote, message: 'Single note fetched Successfully!' });
            } catch (err) {
                res.status(500).json({ status: 'failed', payload: null, message: err });
            }
        } else {
    ...
    api.put('/note/:id', async (req, res) => {
        const id = req.params.id;
        const { title, content, } = req.body;
        if (id) {
            try {
                const updatedNote = await Controller.update(id, content, title );
                res.status(200).json({ status: 'success', payload: updatedNote, message: 'note Updated Successfully!' });
            } catch (err) {
                res.status(500).json({ status: 'failed', payload: null, message: err });
            }
        } else {
    ...
    //deleteTodo
    api.delete('/note/:id', async (req, res) => {
        const id = req.params.id;
        if (id) {
            try {
    ...
    api.delete('/note', async (req, res) => {
        try {
            const deleteTodos = await Controller.deleteAllNotes();
            res.status(200).json({ status: 'success', payload:  deleteTodos, message: 'All note deleted successfully' });
